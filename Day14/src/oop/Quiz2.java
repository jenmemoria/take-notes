...
class Customer{
	...
	// 생성자를 활용하여 매개변수로 이름과 니이를 전달받은 후 필드에 저장하도록 작성하세요.
	...
	boolean isAdult() {
		boolean answer;		// 지역변수
		answer = age >= 20;	// age는 지역변수로 존재하지 않아서 멤버필드를 참조한다. (지역변수가 있으면 그걸 우선 참조함.)
		return answer;		// 함수 영역 내에서 가까운 요소를 참조한다.
	}
	
	// 7) 생성자를 추가로 작성하여 (오버로딩), 이름만으로도 객체를 생성할 수 있도록 한다.
	// 이 때, 값을 초기화하지 않은 age의 값은 0이다.
	// primitive type에는 null이 들어갈 수 없다.
...
public class Quiz2 {
	public static void main(String[] args) {
		...
		Customer cu1 = new Customer("김유경", 22);
		Customer cu2 = new Customer("조예현", 50);
		Customer cu3 = new Customer("이채원", 20);
		Customer cu4 = new Customer("둘리");
		cu1.show();
		cu2.show();
		cu3.show();
		cu4.show();
...
//		6) 손님 객체 3명을 배열로 묶은 다음
		...
		for(int i = 0; i < arr.length; i++) {
			...
			System.out.println();		// 구분을 위하여 한 줄 바꾸기
		}
	}
}
