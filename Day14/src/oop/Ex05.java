...
class PhoneBook {
	...
	// 변수 (member field)	: 객체가 종료되도 쭉 유지되는 변수이다.
	String name;
	int age;
	String phoneNumber;
	...
	PhoneBook(String name, int age, String phoneNumber){
		...
		this.age = age;			// this는 자기자신을 의미.
		this.phoneNumber = phoneNumber;
	}
	
	// 생성자를 오버로딩(중복정의)하며, 매개변수를 전달하지 않아도 객체를 생성할 수 있게 한다.
...
public class Ex05 {
	public static void main(String[] args) {
		...
		// 자바에서는 몇 가지 예외상황을 제외하면, 생성자를 통해서만 객체를 생성할 수 있다.
		// 아래 코드에서 ob1은 객체가 생성되었지만
		// ob2는 객체를 가리킬 수 있는 참조변수만 생성되었을뿐, 실제 객체는 생성되지 않은 상태이다.
		PhoneBook ob1 = new PhoneBook();	// new 뒤가 생성자
		PhoneBook ob2;		// 가리키는 대상이 없는 것.
		
		// 객체를 생성한 직후에는 객체 내부 멤버필드의 값이 0 혹은 null로 초기화되어 있다.
		System.out.println("ob1.name : " + ob1.name);		// "."은 최우선 연산자, "~의"
		System.out.println("ob1.age : " + ob1.age);
		System.out.println("ob1.phoneNumbre : " + ob1.phoneNumber);
		System.out.println();
		...
		for(int i = 0; i < arr.length; i++) {
			arr[i].show();	// 두개 객체를 단일 for문으로 할 수 있다.
		}
		System.out.println();
