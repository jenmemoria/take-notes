...
class Test{
	...
	// 생성자도 함수이므로, 오버로딩(이름이 같지만, 매개변수 형태가 다른 함수 중복 정의)이 가능하다.
	Test(){
...
//		Test ob1 = new Test();	// 생성자를 아예 작성핮 ㅣ않아도 기본 생성자를 호출할 수 있다.
//		ob1.show();
//		
//		ob1.num = 12;
//		ob1.show();
...
/*
 		...
 		3) 클래스 작성 시 어떤 형태이든 생성자를 작성하면, 자동으로 생성되지 않는다.
 			-> 이를 이용하여, 특정 타입이 객체를 생성하기 위해서는 반드시 어떤 값을 전달해야 한다는 규칙이 만들어진다.
 			-> 아무값을 만들어주지 않으면 만들어지지 않는다.	ex) 스캐너
 ...
 */		// Scanner 클래스는 객체를 생성하기 위해서, InputStream 혹은 String 혹은 File 등을 전달해야 한다.
