...
class Test2{
	...
	void showAdd() {	// 5) ob 객체의 메서드가 호출되면 아래 내용을 실행한다.
		System.out.printf("%d + %d = %d\n", x, y);
		// 6) 출력함수 printf를 호출하여 실행한다.
		
	}
}
// this.[멤버]		멤버 필드 및 멤버 메서드에 접근한다.
// this()			클래스 생성자 내부에서 또 다른 생성자를 호출할 때 사용한다. *첫줄에서만 사용 가능
...
public class Ex02 {
	public static void main(String[] args) {
		...
		ob.x = 10;		// 2) ob 객체인 x필드에 10을 대입한다.
		ob.y = 20;		// 3) ob 객체의 y필드에 20을 대입한다.
		
		ob.showAdd();	// 4) ob 객체의 showAdd 메서드를 호출한다.
