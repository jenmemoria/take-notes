...
class Airplane{
	...
	public boolean entrance(Human guest) {		// 손님이 입장을 시도하면
		guest.info();// 업캐스팅 상태에서 오버라이딩 메서드를 호출한다.		없어도 여기선 ok
	...
	public void simulationEmergency(int index) {
		...
		// 의사가 있는지 없는지 먼저 판별
		// 의사가 없으면 없다고 출력
		// 있으면 의사의 cure 메서드를 호출하면서 환자 객체를 전달.
...
//		for (int i = 0; i < seat.length; i++) {
//			String jobName = seat[i].getClass().getSimpleName();
////			seat[i].info();
//			
//			switch(jobName) {
//			case "Doctor" :
//				Doctor tmp1 = (Doctor) seat[i];
//				tmp1.cure(seat[index]);
//				break;
//			default :
//				break;
//			}
...
public class Ex02 {
	public static void main(String[] args) {
		...
		// 서브클래스에 의해 생성된 객체를 배열에 넣을 때, 업캐스팅이 발생한다.		업캐스팅은 암묵적으로 형변환이 발생한다.
...
//		}
		...
		// 슈퍼클래스를 배열에 담거나, 슈퍼클래스 타입의 매개변수에 전달하면 업캐스팅이 발생한다.
		// 업캐스팅 상태에서도 오버라이딩 메서드는 바로 호출하여 실행하면 된다.	** 강점
