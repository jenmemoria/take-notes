...
//		a1.play();	// 이 상태에서는 play()가 안 된다 -> Animal 클래스에는 play() 메서드가 없다. (Cat의 고유기능)
//		a2.play();	// Animal 클래스에는 tailing() 메서드가 없다. (Dog의 고유기능)
		
		a1.bark();	// 슈퍼클래스 Animal에는 name + " : ..."으로 정의되어 있으나
		a2.bark();	// 각 서브클래스의 오버라이딩 내용으로 실행된다.
		
//		upcasting : 서브클래스의 객체를 슈퍼클래스로 참조를 한 형태 (자료형 변환)	-> 굳이 하는 이유 : 일괄처리를 하기 위해서
