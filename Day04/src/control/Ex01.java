...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 비교연산 : >, <, >=, <=, ==, !=			일반적으로 수치에 대한 비교를 수행하여 boolean을 만들어낸다.
		// 논리연산 : &&, ||, !					boolean을 이용하여 새로운 boolean을 만들어낸다.
		
		// 사용자의 "입력값"이 무엇이냐에 따라, 어떤 크기의 값이냐에 따라
		// 조건을 생서하고, 조건에 따라 서로 다른 코드를 수행하게 된다.
		...
		if(isOdd) {		// 조건에 따라 코드 수행 (입력값이 홀수이면)
			num++;		// 값을 1 증가시킨다. (짝수로 만든다.)
		}
		
		while(true) {	// 무한반복 (조건이 항상 참이기 때문에, 계속 반복된다.), 조건문과 반복문을 같이 가지고 있다.
			num /= 2;	// num값을 2로 나누고, 나눈 값을 다시 num에 대입한다.
			if(num < 10) {	// 만약 num의 값이 10보다 작으면 (한 자리 정수이면)
