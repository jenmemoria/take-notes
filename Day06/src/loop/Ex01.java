...
public class Ex01 {
	public static void main(String[] args) {
		...
		int num = 0;		// 반복할 횟수를 체크할 변수
		while(num < 5) {	// 반복을 수행할 조건
			num++;			// 횟수를 1 증가시킨후
		...
		for(int i = 0; i < 25; i++) {				// 반복의 횟수 선언, 조건, 증감
			System.out.println("for : " + i);		// 종속 문장
		}
		System.out.println();
		...
		if(num < 20) {	// 조건이 참일 때, 종속 문장을 1회 실행한다.
			System.out.println("if : " + ++num);
		}
		while(num < 20) {	// 조건이 참인동안 종속 문장을 반복하여 실행한다.
			System.out.println("while : " + ++num);
		}	// 종속 문장을 실행하고 나면, 다시 조건을 체크한다.
	}
}
