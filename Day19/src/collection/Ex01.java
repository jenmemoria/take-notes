...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 리스트에 값을 추가하려면 add 함수를 이용한다.
		// list.add(e) 함수는 리스트의 가장 마지막에 요소를 추가한다.
		list.add(10);
		list.add(20);
		
		// 리스트 및 컬렉션은 대부분 toString()이 오버라이딩 되어 있어서 바로 출력해도 내용을 확인할 수 있다.
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
		// list.add(int index, E e) 함수는 지정한 인덱스에 값을 추가한다.
		list.add(1, 30);	// add 함수는 오버로딩이 됐다 !
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
...
//		list.remove(30);						// 정수를 그냥 전달하면 index로 간주한다.
		...
		list.add(40);
		list.add(50);
		System.out.println(new int[] {20, 40, 10, 50, 30});
		
		System.out.println(list);	// tostring이 되어져 있어서 바로 출력 가능
		System.out.println("list.get(0) : " + list.get(0));
		System.out.println("list.get(1) : " + list.get(1));
		System.out.println("list.get(2) : " + list.get(2));
		
		// list.add 함수는 값을 추가하는데 사용하지만
		// list.set(int index, E e) 함수는 지정한 위치의 값을 바꿀 때 사용한다.
		list.set(0, 60);
		System.out.println(list);
		
		// 정렬함수가 내장되어 있거나, 다른 클래스를 이용하여 손쉽게 정렬할 수 있다.
		list.sort(null);	// null 자리에는 정렬 기준을 나타내기 위한 객체를 전달할 수 있다, 반드시 null 값을 넣어줘야 한다.
		System.out.println(list);
		...
		// 어떤 리스트가 주어질 때, 비어있는지 확인하기 위한 메서드도 있다.
		System.out.println("list가 비어있는 가 : " + list.isEmpty());
		...
		// 리스트 내부에 특정값이 포함되어있는지 판별하는 함수도 있다.
