...
//			System.out.printf("%s : %,d원\n", b.name, b.price);
	...
	public static void main(String[] args) {
		...
		// 이 때, 선택정렬 코드의 if 조건에 해당하는 부분만 만들어내는 함수가 compare 혹은 compareTo이다.
		// if(arr[i] - arr[j] > 0) { ... }
		list.sort((b1, b2) -> {
...
//			System.out.printf("[%s]와 [%s]를 비교한 차이값 : %d\n", b1.name, b2.name, diff);
		...
		// Comparable : 클래스에서 implements 해서 사용하는 인터페이스	우리가 만든 북은 comparable 하지 않음. 
		// ㄴ> 나랑 다른 객체를 비교하는 것.
		// class Book implements Comparable {비교 가능한 도서 (compareTo를 오버라이딩한다.)	}
		
		// Comparator : 람다식으로 객체를 생성하여 전달할 때 사용한다.	외부에서 두 객체를 비교                                                                                                                                                                                                            
		// 비교함수가 없거나, 자신만의 기준으로 비교하고 싶을 때 만들어서 전달한다.
		list.sort(null);	// null을 넣으면 예외가 발생함.
		showList(list);
		
		// Comparable와 Comparator 비교하기 !!! 
