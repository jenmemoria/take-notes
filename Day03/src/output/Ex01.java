...
public class Ex01 {
	public static void main(String[] args) {
		/* 자바의 기본 출력 함수 3가지
		 System.out.println() : 괄호안에 하나의 값을 받아서 출력한다. 출력후 한 줄 내린다.
		...
		System.out.println("b");		// 괄호 안의 계산이 먼저 진행되고			System.out.println('b'); 이렇게 하면 캐릭터로 생각함.
		...
		System.out.print("\r");		// \r은 커서를 줄 왼쪽 끝으로 이동시킨다. (CarrigeReturn)
		System.out.print("C");		// \r도 엔터의 값이므로 줄을 바꾼다.
		System.out.println();		// 보통 줄만 바꾸고 싶으면 println()을 사용한다.
		
		String lyrics = "동해물과 백두산이 마르고 닳도록\r\n";
		lyrics += "하느님이 보우하사 우리나라 만세\r\n";
		
		// + 연산자는 숫자간의 연산에서 숫자를 계산하고
		// 문자열이 양쪽 중 한 곳에 포함되면, 문자열을 베이스로 하여 글자를 이어붙이기 한다. (append)
		// 산술연산자는 복합대입연산자 형식으로 사용할 수 있기 때문에
		// lyrics에 처음 값을 지정한 후
		// 기존 값을 유지하면서 새로운 내용을 덧붙이기(append)할 때 사용할 수 있다.
		...
		//	print와 println의 자치는 주로 반복문 내부에서 값을 연속으로 출력할 때 활용한다.
		
		for(int i = 0; i < 5; i++) {
			System.out.print(i + " ");		// i를 출력하고 한 칸 띄어쓰기 하고 줄은 바꾸지 않기.
		// 출력하면 int와 string을 더한 것이므로 출력한 것은 string이다.
		}
		...
		int num = 1234;
		String num2 = 1234 + "";
		// String이 아닌 값을 가장 쉽게(효율적인 방법은 아님) 문자열로 바꾸는 방법은
